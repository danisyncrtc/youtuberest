...
module.exports = (function() {
  // default config
  var _config = {
    format: `<<${loggerServ}>> {{timestamp}} <{{title}}> {{file}}:{{line}} ({{method}}) {{message}}`,
  ...
  const fLen = _config.filters.length;
  var lastFilter;
  if(fLen > 0) {
  ...
  _config.methods.forEach(function(title, i) {
    if(i < _config.level) {
      ...
      const needstack = /{{(method|path|line|pos|file)}}/i.test(format);
